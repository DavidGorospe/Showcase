...
Usage:
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    // clear screen and set viewport (dark space with subtle nebula tint)
    // Animate the background color slightly for nebula effect
    float bg_time = animation_seconds * 0.1;
    float nebula_tint = 0.5 + 0.5 * sin(bg_time);
    glClearColor(0.01 + 0.01 * nebula_tint, 0.01 + 0.02 * nebula_tint, 0.05 + 0.03 * nebula_tint, 1.0);
